...
def set_git_remote(local_path, username, token, remote_url, email='tgoetz@inductiveautomation.com', branch='main'):
    """
    ...
    try:
        ...
        subprocess.check_call(["git", "branch", "-M", branch], cwd=local_path)
        logger.info("Set the default branch to '{}'".format(branch))
...
def project_update_from_git(repo_url, branch, username, token, projectsToUpdate=[], gitSubFolder=""):
	'''
	...
	if temp_repo_path:
		...
		try:
			...
			return False
		if gitSubFolder!="":
			gitProjectPath="{}/{}".format(temp_repo_path,gitSubFolder)
		else:
			gitProjectPath=temp_repo_path
		listProjects=fileSystem.list_directories(gitProjectPath)
